...
import sys
dir_path = str(Path(__file__).resolve().parent.parent.parent.parent)
...
from deep_research_agent.prompt.score_prompt_zh import generate_static_score_system_prompt_zh, generate_static_score_user_prompt_zh
# from src.openrlhf_agent.agentkit.prompt.score_prompt_en import generate_static_score_system_prompt_en, generate_static_score_user_promptc_en
from deep_research_agent.reward.rubric import static_rubric
from deep_research_agent.reward.example import article
...
class ReportRewardStrategy(ResultRewardStrategy):
    ...
    async def generate_score(self, question: str, clean_response: str, clean_label: str, max_retries=3):
        system_prompt, user_prompt = generate_static_score_system_prompt_zh, generate_static_score_user_prompt_zh
        user_prompt = user_prompt.format(task_prompt=question, article_1=clean_response, article_2=clean_label)
        llm_response_str = ""
    ...
    async def score_response(self, question: str, response: str, label: str) -> float:
        try:
            ...
            if clean_response is None or clean_label is None:
                return 0.0

            json_str_extracted = await self.generate_score(question, clean_response, clean_label)
            reward = self.cal_reward(json_str_extracted, static_rubric)
            return reward
        except Exception as e:
            return 0.0
    ...
    async def score(
        ...
        *,
        ...
        # 若缺少 问题、回答、标签 任一种返回零
        if label is None or response is None or question_text is None:
            return self.miss_score

        return await self.score_response(question_text, response, str(label))
